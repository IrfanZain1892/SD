...
export const News: React.FC = () => {
  ...
  return (
    <div className="flex flex-col">
      ...
      <section className="py-16 bg-[#F0F4FF] min-h-screen">
        <div className="container mx-auto px-4">
          {/* Filters & Search */}
          <div className="flex flex-col lg:flex-row items-center justify-between gap-8 mb-12">
            <Tabs defaultValue="Semua" className="w-full lg:w-auto" onValueChange={setActiveTab}>
              <TabsList className="bg-[#F0F4FF] p-1 rounded-2xl shadow-sm w-full lg:w-auto flex flex-wrap h-auto">
                {categories.map(cat => (
                  <TabsTrigger 
                    key={cat} 
                    value={cat} 
          ...
          {filteredNews.length > 0 && (
            <div className="flex justify-center space-x-2">
              <Button variant="outline" className="rounded-xl w-10 h-10 p-0 border-surface bg-[#F0F4FF]">1</Button>
